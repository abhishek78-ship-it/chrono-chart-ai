...
export const Route = createFileRoute("/report/")({
  head: () => ({ meta: [{ title: "Reports — AI Dataset Detective" }] }),
...
function ReportIndex() {
  ...
  if (datasets.length === 1) {
    return <Navigate to="/report/$id" params={{ id: datasets[0].id }} />;
  }
  return (
    <AppShell>
      <h1 className="font-display text-3xl font-bold md:text-4xl">Reports</h1>
      <p className="mt-2 text-muted-foreground">
        Choose a dataset to preview and download its PDF report.
      </p>
      {datasets.length === 0 ? (
        <div className="glass mt-8 rounded-3xl p-12 text-center">
          ...
          <h3 className="mt-5 font-display text-xl font-semibold">No reports yet</h3>
          <Link
            to="/upload"
